...
all_participants_dir = './participants'
...
for pid in range(1, ids + 1):
    if pid not in pids_personas:
        fit_data_pid = pd.read_csv('./participants/pid-' + str(pid) + '/health_fitness_dataset_pid-' + str(pid)
                                   + '_training.csv').convert_dtypes()
        for m in range(1, 13):
            if m <= 9:
            ...
            month.to_csv(single_participant_dir + str(pid) + '/health_fitness_dataset_pid-' + str(pid)
                         + '_month-' + str(m) + '_training.csv', index=False)
# for each participant, create testing a separate dataset for each month
for pid in range(1, ids + 1):
    fit_data_pid = pd.read_csv('./participants/pid-' + str(pid) + '/health_fitness_dataset_pid-' + str(pid)
                               + '_testing.csv').convert_dtypes()
    for m in range(1, 13):
        if m <= 9:
            month = fit_data_pid[fit_data_pid['date'].str.contains('2024-0' + str(m))]
        else:
            month = fit_data_pid[fit_data_pid['date'].str.contains('2024-' + str(m))]
        month.to_csv(single_participant_dir + str(pid) + '/health_fitness_dataset_pid-' + str(pid) + '_month-' + str(
            m) + '_testing.csv', index=False)
    ...
    if pid not in pids_personas:
        for m in range(1, 13):
            fit_data_pid_averaging = pd.read_csv(
                './participants/pid-' + str(pid) + '/health_fitness_dataset_pid-' + str(pid) + '_month-' + str(
                    m) + '_training.csv')
            ...
            csv_filename = single_participant_dir + str(pid) + '/health_fitness_dataset_pid-' + str(
                pid) + '_month-' + str(m) + '_averaged_training.csv'
...
for pid in range(1, ids + 1):
    if pid not in pids_personas:
        for m in range(1, 13):
            data_names_training.append(
                single_participant_dir + str(pid) + '/health_fitness_dataset_pid-' + str(pid) + '_month-' + str(
                    m) + '_averaged_training.csv')
...
for pid in range(1, ids + 1):
    for m in range(1, 13):
        data_names_testing.append(
            single_participant_dir + str(pid) + '/health_fitness_dataset_pid-' + str(pid) + '_month-' + str(
                m) + '_averaged_testing.csv')
...
to_be_encoded = pd.read_csv('./datasets/regularised_averaged_health_fitness_dataset_training.csv')
...
to_be_encoded = pd.read_csv('./datasets/regularised_averaged_health_fitness_dataset_testing.csv')
encoded_dataset = pd.get_dummies(data=to_be_encoded, columns=['gender', 'activity_type', 'intensity',
                                                              'health_condition', 'smoking_status'], dtype='int8')
...
to_be_converted = pd.read_csv('./datasets/regularised_averaged_health_fitness_dataset_training.csv')
...
to_be_converted = pd.read_csv('./datasets/regularised_averaged_health_fitness_dataset_testing.csv')
to_be_converted.replace(['F', 'M', 'Other'], [0, 1, 2], inplace=True)
to_be_converted.replace(['Never', 'Current', 'Former'], [0, 1, 2], inplace=True)
to_be_converted.replace(['None', 'Hypertension', 'Diabetes', 'Asthma'], [0, 1, 2, 3], inplace=True)
non_numeric_columns = list(to_be_converted.select_dtypes(exclude=[np.number]).columns)
