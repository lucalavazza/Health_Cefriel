...
for pid in range(1, ids+1):
    for m in range(1, 13):
        fit_data_pid_test = pd.read_csv('/Users/luca_lavazza/Documents/GitHub/Health_Cefriel/participants/pid-'+str(pid)+'/health_fitness_dataset_pid-'+str(pid)+'_month-'+str(m)+'.csv')
        non_numeric_columns = list(fit_data_pid_test.select_dtypes(exclude=[np.number]).columns)
        label = LabelEncoder()
        for col in non_numeric_columns:
            fit_data_pid_test[col] = label.fit_transform(fit_data_pid_test[col])

        column_avgs = fit_data_pid_test.mean()
        list_indexes = column_avgs.index.tolist()
        list_values = column_avgs.to_list()

        averaged_fit_data_pid_test = {}
        special_treatment_cols = ['date', 'gender', 'activity_type', 'health_condition', 'smoking_status']
        for col in fit_data_pid_test.columns:
            if col not in special_treatment_cols:
                averaged_fit_data_pid_test[col] = round(column_avgs[col], 2)
            else:
                if col == 'gender':
                    averaged_fit_data_pid_test[col] = fit_data_pid_test['gender'][0]
                elif col == 'date':
                    if m <= 9:
                    ...
                    averaged_fit_data_pid_test[col] = '#TODO'
...
# TODO: take care of special_treatment_cols
