...
from pipeline.config import LABELLED_TEST_DATASET, LABELLED_TRAIN_DATASET, PIDS_PERSONAS, PREPROCESSING_METADATA_PATH, RANDOM_SEED, load_causal_graph
...
PREPROCESSING_METADATA = load_preprocessing_metadata(PREPROCESSING_METADATA_PATH) if Path(PREPROCESSING_METADATA_PATH).exists() else None

data = pd.read_csv(LABELLED_TRAIN_DATASET)
data_testing = pd.read_csv(LABELLED_TEST_DATASET)
...
SCM_TXT_PATH = OUTPUT_PATH / "scm.txt"
SCM_COEFS_PATH = OUTPUT_PATH / "scm_coefficients.json"
CF_RESULTS_PATH = OUTPUT_PATH / "cf_results.json"
AGREEMENT_JSON_PATH = OUTPUT_PATH / "linear_vs_gcm_mae_original_units.json"
AGREEMENT_CSV_PATH = OUTPUT_PATH / "linear_vs_gcm_mae_original_units.csv"
...
def build_design_matrix(data: pd.DataFrame, parent_vars: List[str]) -> pd.DataFrame:
    if len(parent_vars) == 0:
        return sm.add_constant(pd.DataFrame(index=data.index), has_constant="add")
...
def render_exogenous_equation(variable_name: str) -> str:
    scm_dict[variable_name] = [(variable_name, 1.0)]
    return f"{variable_name} = (1.0000*{variable_name})"
...
def build_scm_parent_map(graph: nx.DiGraph):
    return {
        node: sorted(list(graph.predecessors(node)))
        for node in nx.topological_sort(graph)
    }


def inverse_transform_counterfactual_results(results_by_pid):
    if PREPROCESSING_METADATA is None:
        return results_by_pid

    converted = {}
    scaling_metadata = PREPROCESSING_METADATA.get("scaled_numeric_columns", {})
    for pid, per_variable in results_by_pid.items():
        converted_pid = {}
        for variable_name, series_or_table in per_variable.items():
            if variable_name not in scaling_metadata or not isinstance(series_or_table, dict):
                converted_pid[variable_name] = series_or_table
                continue

            if any(str(key).isdigit() for key in series_or_table.keys()):
                ordered_keys = list(series_or_table.keys())
                ordered_values = [series_or_table[key] for key in ordered_keys]
                inverse_values = inverse_transform_values(ordered_values, variable_name, PREPROCESSING_METADATA)
                converted_pid[variable_name] = {
                    key: float(value) for key, value in zip(ordered_keys, inverse_values)
                }
            else:
                converted_pid[variable_name] = series_or_table
        converted[pid] = converted_pid
    return converted
...
def compute_linear_vs_gcm_mae(linear_results, gcm_results, excluded_variables=None):
    excluded_variables = excluded_variables or set()
    errors_by_variable = {}
    linear_by_pid = {pid_key(k): v for k, v in linear_results.items()}
    gcm_by_pid = {pid_key(k): v for k, v in gcm_results.items()}

    for pid, gcm_block in gcm_by_pid.items():
        if pid not in linear_by_pid:
            continue
        linear_block = linear_by_pid[pid]
        for variable_name in sorted(set(gcm_block.keys()) & set(linear_block.keys())):
            if variable_name in excluded_variables:
                continue
            gcm_series = get_series(gcm_block, variable_name)
            linear_series = get_series(linear_block, variable_name)
            if not isinstance(gcm_series, dict) or not isinstance(linear_series, dict):
                continue
            common_indices = sorted(set(gcm_series.keys()) & set(linear_series.keys()), key=str)
            for idx in common_indices:
                try:
                    gcm_value = float(gcm_series[idx])
                    linear_value = float(linear_series[idx])
                except Exception:
                    continue
                if math.isnan(gcm_value) or math.isnan(linear_value):
                    continue
                errors_by_variable.setdefault(variable_name, []).append(abs(gcm_value - linear_value))

    rows = []
    for variable_name in sorted(errors_by_variable.keys()):
        variable_errors = errors_by_variable[variable_name]
        if not variable_errors:
            continue
        rows.append({
            "variable": variable_name,
            "mae_original_units": sum(variable_errors) / len(variable_errors),
            "n_observations": len(variable_errors),
        })
    return rows


def write_agreement_summary(rows, json_path: Path, csv_path: Path):
    json_payload = {
        row["variable"]: {
            "mae_original_units": row["mae_original_units"],
    ...
    with open(csv_path, "w", newline="") as file:
        writer = csv.DictWriter(file, fieldnames=["variable", "mae_original_units", "n_observations"])
        writer.writeheader()
...
# Cannot use AssignmentQuality = BEST due to compatibility issues with python 3.8
print('\n*** SCM with AssignmentQuality = BETTER\n')
causal_model = InvertibleStructuralCausalModel(G)
_ = gcm.auto.assign_causal_mechanisms(causal_model=causal_model, based_on=data, quality=AssignmentQuality.BETTER)
print('\n')
_ = gcm.fit(causal_model=causal_model, data=data, return_evaluation_summary=True)
SCM = build_scm_parent_map(G)
with open(SCM_TXT_PATH, 'w') as f:
    json.dump(SCM, f, indent=4)
print('\n*** SCM computed.\n')
...
for dep_var, parent_vars in SCM.items():
    if dep_var not in data.columns:
        continue
    if len(parent_vars) == 0:
        equations.append(render_exogenous_equation(dep_var))
        continue
...
with open(SCM_COEFS_PATH, 'w') as file:
...
duration_intervention_original = compat_standardized_value_to_original(
    -3.0,
    "duration_minutes",
    PREPROCESSING_METADATA,
) if PREPROCESSING_METADATA is not None else -3.0
duration_intervention_standardized = to_standardized_value(duration_intervention_original, "duration_minutes")
for pid in pids_personas:
    ...
    counterfactual_data_pids[pid] = gcm.counterfactual_samples(causal_model,
                                                        {'duration_minutes': lambda x, value=duration_intervention_standardized: value},
                                                        observed_data=fitness_data_pids[pid])
    counterfactual_results_pids[pid] = counterfactual_data_pids[pid].to_dict()
with open(CF_RESULTS_PATH, 'w') as file:
    file.write(json.dumps(counterfactual_results_pids, indent=4))
with open(OUTPUT_PATH / "cf_results_original_units.json", 'w') as file:
    file.write(json.dumps(inverse_transform_counterfactual_results(counterfactual_results_pids), indent=4))
...
intervention = {"duration_minutes": duration_intervention_standardized}
eq_results = compute_linear_scm_counterfactuals(data_testing, pids_personas, SCM, scm_coefficients, intervention,
                                                cf_results, pid_col="participant_id", date_col="date",)
with open(OUTPUT_PATH / "linear_cf_results.json", "w") as f:
    json.dump(eq_results, f, indent=4)
linear_results_original = inverse_transform_counterfactual_results(eq_results)
with open(OUTPUT_PATH / "linear_cf_results_original_units.json", "w") as f:
    json.dump(linear_results_original, f, indent=4)
print('Linear SCM values computed')
print(50*'-')

gcm_results_original = inverse_transform_counterfactual_results(cf_results)
agreement_rows = compute_linear_vs_gcm_mae(
    linear_results_original,
    gcm_results_original,
    excluded_variables={"participant_id", "date", "gender", "activity_type", "intensity", "smoking_status", "health_condition"},
)
write_agreement_summary(agreement_rows, AGREEMENT_JSON_PATH, AGREEMENT_CSV_PATH)
print('Linear-vs-GCM agreement summary exported')
print(50*'-')

# Computing the epsilons
with open(OUTPUT_PATH / "linear_cf_results.json", "r") as f:
...
with open(OUTPUT_PATH / "epsilon_results.json", "w") as f:
    json.dump(epsilons, f, indent=4)

with open(OUTPUT_PATH / "epsilon_results.json", "r") as f:
    eps = json.load(f)
avg_eps = average_epsilons_over_dates(eps, drop_empty=True)
with open(OUTPUT_PATH / "epsilon_means_by_pid_and_var.json", "w") as f:
